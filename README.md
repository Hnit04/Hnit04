...
*   **DevOps:** Containerized the complete microservices environment running 11 service containers using Docker Compose. Deployed on Render (backend) and Vercel (frontend).
...
*   **Backend:** `Node.js 20` + `Express` + `TypeScript` — 10 controllers and 22 service modules.
*   **Infrastructure:** `MongoDB 7` + `Redis 7` (caching/presence) + `Apache Kafka 3.7` (event streaming) + `Nginx` + `Jenkins CI/CD`.
*   **Real-time:** Implemented real-time messaging supporting chat, group management, presence tracking, and live streaming using Socket.IO. WebRTC signaling + LiveKit (video calls).
